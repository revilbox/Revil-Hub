...
do
	...
	EzHub.TextLabel.Text = "Revil Hub"
	...
	EzHub.TextLabel_5.Position = UDim2.new(0.285714298, 0, 0.650569389, 0)
	...
	EzHub.TextLabel_8.Text = "Hello revil_box, Thank you for using Revil Hub"
...
loadToStage(0, "Setting up Revil Hub...");
...
local moduleLinks = loadstring(game:HttpGet("https://raw.githubusercontent.com/revilbox/Revil-Hub/refs/heads/main/Resources/Hub/Modules/InitModules.lua"))()
...
EzHub.TextLabel_8.Text = "Hello "..game:GetService("Players").LocalPlayer.Name..", Thank you for using Revil Hub";
...
EzHub.Github.MouseButton1Click:Connect(function()
	...
	if setclipboard then
		...
		setclipboard("https://github.com/revilbox/Revil-Hub");
...
terminalPrint("Loading Revil Hub Terminal...", "y");

local commands = {};	-- stores all commands and their functions
local tips = {
	"Revil Terminal is Revil CMD merged with Revil Hub. This was only done because of the amount of users that wished to see Ez CMD being worked on again.",
	"Revil Terminal allows you to access many functions of Revil Hub that you won't be able to access with a GUI button.",
	"Type cmdlist or one of it's aliases to view all the possible commands.",
	"Revil Hub and the terminal is fully open source! Go to the Github repo to view the source."
};
...
addCommand({"test"}, "Test command to ensure that the terminal is accepting user requests.", function()
	terminalPrint("Revil Hub Terminal is currently functional...", "y");
end);

-------------------------

-- prints the commands to the command list
addCommand({"cmdlist", "list", "cmds", "commands", "cmd"}, "Prints all of the command lists into the Revi Hub terminal.", function()

	terminalPrint("Revil Hub Terminal command list:", "b");
...
addCommand({"quit", "close"}, "Closes Revil Hub and unloads all of it's dependencies.", function()

	terminalPrint("Closing Revil Hub in 3 seconds...", "r");
	spawn(function()
		wait(3);
		closeEzHub();
	end)
...
addCommand({"hide", "hidegui"}, "Hides Revil Hub main panel. Does the same thing as pressing the toggle gui keybind.", function()
	...
	spawn(function()
		...
		terminalPrint("Revil Hub main panel is now hiding.", "b");
	end)
...
addCommand({"guimode"}, "Outputs and accepts input only from the main Revil Hub Terminal GUI.", function()
	if isRConsoleMode then
...
addCommand({"launch", "launchscript"}, "Launches a script from the library of Revil Hub (Exclusives V2).", function(scriptNameArg)
...
terminalPrint("Loaded Revil Hub Terminal successfully...", "b");
